...
type Stops = Array<[altitude: number, color: [number, number, number]]>;
...
// Decorative stars, fixed in the frame. They are not a catalogue and do not
// wheel with the hour, so they only show once it is properly dark and are kept
// faint enough not to be mistaken for one.
const STARS = (() => {
  let seed = 20260924;
  const next = () => {
  ...
  return Array.from({ length: 140 }, () => {
    ...
    const radius = Math.sqrt(next()) * (R - 6);
    return { x: Math.cos(angle) * radius, y: Math.sin(angle) * radius, r: 0.5 + next() * 1.1, o: 0.35 + next() * 0.65 };
  });
})();
    ...
    >
      <defs>
        <radialGradient id="sky-fill" cx="0" cy="0" r={R} gradientUnits="userSpaceOnUse">
          <stop offset="0" stopColor={ramp(ZENITH, sun.altitude)} />
          <stop offset="0.62" stopColor={ramp(ZENITH, sun.altitude - 2)} />
          <stop offset="1" stopColor={ramp(HORIZON, sun.altitude)} />
        </radialGradient>
      ...
      {sun.altitude < -4 && (
        <g fill="#fff" opacity={Math.min(1, (-sun.altitude - 4) / 12)}>
          {STARS.map((star, i) => (
            <circle key={i} cx={star.x} cy={star.y} r={star.r} fillOpacity={star.o} />
          ))}
        </g>
      )}
      ...
      {marks.map(({ pad, launch, position, glow }) => {
        ...
        const text = `${launch.name}${pad.launches.length > 1 ? ` (+${pad.launches.length - 1} more here)` : ''}`;
